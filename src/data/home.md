---
# Site name — used in <title>, header .logo, footer copyright
name: bitácora

# Meta description — used in <meta name="description"> and RSS feed
description: "Apuntes de operaciones ofensivas, reverso de protocolos Windows y desarrollo de herramientas para red team. Kerberos, MS-RPC, AD internals y evasión de EDR."

# Site base URL — used by RSS feed generation (must match deployment URL)
url: "https://example.com"

# Language code — set on <html lang=…>
lang: es

# Author name — shown in footer copyright
author: z4d3s

# Social links — rendered as underlined pills in hero section
github: https://github.com/z4d3s
twitter: https://twitter.com/z4d3s
mail: z4d3s@bitacora.xyz
---

<!-- Bio text — rendered as the hero paragraph on index.astro -->
Red teamer aprendiendo y compartiendo. Desarmo protocolos, escribo tooling y evito firmas.
